...
alignAllLines d n = unlines.(map unwords).(map (\(l, ws) -> align d n l ws)).(allLinesWithWordsPerLine n).concat.(map words).lines
  where
    align 'l' num spaceConsumed words = init words ++ [(last words) ++ (take (num - spaceConsumed) (repeat ' '))]
    align 'r' num spaceConsumed words = (reverse.(map reverse).(align 'l' num spaceConsumed).(map reverse).reverse) words
