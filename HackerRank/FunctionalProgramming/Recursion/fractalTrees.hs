...
fractalize :: Int -> Int -> Int -> [[Int]]
fractalize pos itnum iter = 
  let myones = numOnes !! (itnum)
      mycss  = fractalYArms myones pos
      recFracLeft  = fractalize (head . last $ mycss) (itnum + 1) (iter)
      recFracRight = fractalize (last . last $ mycss) (itnum + 1) (iter)
      moreRows = combineTwoFractals recFracLeft recFracRight
  in  if iter == itnum then [] else mycss ++ moreRows
...
main = do
  nstr <- getLine 
  let css = reverse . fillUnderScores . map lineToChars $ fractalize (numCols `div` 2) 0 (read nstr)
