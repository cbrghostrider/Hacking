...
private:
    ...
    Ways computeWays(int fromdim, int remsteps);
public:
...
Ways HyperCuboid::computeWays(int fromdim, int remsteps) {
    if (remsteps <= 0) {            //base case, can't move anymore
        return 1;   
    }
    if (fromdim == dims.size()-1) { //base case, no more dims left to recurse on 
        return dimCache[fromdim].waysForSteps(remsteps);
    }

    //recursive case
    Ways totways = 0;
    for (int s=0; s<=remsteps; s++) { //I will try from [0..remsteps] number of steps in my dimension
        //will walk s steps in my dimension, and remsteps-s steps in remaining dimensions
        Ways myways = dimCache[fromdim].waysForSteps(s);
        Ways recurseways = computeWays(fromdim+1, remsteps-s);
        unsigned long long lways = ((unsigned long long) myways ) * ((unsigned long long) recurseways);
...
Ways HyperCuboid::computeWays() {
    Ways ways = computeWays(0, nsteps);
