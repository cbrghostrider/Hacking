...
getFlowDirForCoord (nrow, ncol) am c = 
  let neighDirAlt = head . sortBy sortOnAlt . map (\(cd, cc) -> (cd, getElem am cc)) . getNeighbors (nrow, ncol) $ c
      myAlt       = getElem am c
  in  if myAlt <= snd neighDirAlt then Nothing else Just (fst neighDirAlt)
...
sortOnLocation :: Coord -> Coord -> Ordering
sortOnLocation cl@(clx, cly) cr@(crx, cry) = (cly `compare` cry) `mappend` (clx `compare` crx)
  where getMagnitude (x, y) = x * c_MAXDIM + y

sinkToLetterMap :: SinkMap -> [(Char, Coord)]
sinkToLetterMap sm = zip labels . nub . sortBy sortOnLocation . concat $ sm
...
processInput = unlines . map (\(n, m) -> processMap n m) . zip [1..] . readInput . tail . lines
